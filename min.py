...
class Wall(sprite.Sprite):
    def __init__(self, wall_x, hole_height, top_image, bottom_image):
        ...
        self.top_wall = transform.scale(image.load(top_image), (50, random.randint(200, win_height - hole_height)))
    ...
    def update(self):
        ...
        if self.top_rect.x < -300:
            self.top_rect.x = win_width
            self.bottom_rect.x = win_width
            self.top_wall = transform.scale(image.load("pipedown.png"), (50, random.randint(40, win_height - self.hole_height)))
...
def restart_game():
    ...
    hole_height = 100
    for i in range(5):
        wall = Wall(win_width + i * 270, hole_height, "pipedown.png", "pipeup.png")
